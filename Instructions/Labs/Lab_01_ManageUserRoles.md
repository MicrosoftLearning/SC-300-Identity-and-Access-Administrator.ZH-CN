...
2. 在左侧菜单中，展开“Entra ID”下拉列表（如果尚未打开）。****
...
3. 更新你的密码。
  ...
  实验室提示 - 建议使用实验室环境中提供的用户密码。****

4. 在屏幕顶部的搜索对话框中搜索并选择“企业应用程序”。

5. 选择“+ 新建应用程序”。 请注意，“+ 创建自己的应用程序”不可用。

6. 尝试选择任何其他设置（如“许可和权限”****）以及“用户设置”**** 来验证 Chris Green**** 没有权限。
...
5. 从“管理”菜单中选择“分配的角色”。
6. 选择“**+ 添加分配**”。
7. 在下拉列表中选择“`Application administrator`”角色。
8. 选择“**下一步**”按钮。
9. 标记**分配类型**的**有效**值。
10. 输入“实验室需要”等理由。 然后，选择“分配”****。
...
1. 启动一个新的 InPrivate 浏览器窗口。
2. 以 Chris Green 的身份通过 +++https://entra.microsoftcom+++ 打开 Microsoft Entra 管理中心。
...
3. 在屏幕顶部的搜索对话框中搜索并选择“企业应用程序”。
4. 请注意，现在可以使用“+ 新建应用程序”。
5. 选择“+ 新建应用程序”
6. 查看“+创建自己的应用程序”是否不再灰显。**** 如果选择库应用，则会看到“创建”按钮可用。****
...
8. 前往 Allfiles/Lab1 文件夹的路径，选择 **SC300BulkUser.csv** 文件。
...
**实验室提示** - TouchType 在实验室环境中不适用于 PowerShell。  要解决此问题，请在实验室环境中打开记事本。 接下来，使用 TouchType 功能将脚本放入记事本，最后使用复制和粘贴将命令放入 PowerShell。  为此额外步骤道歉。
...
4. 接下来，需要通过运行以下命令登录到 Microsoft Graph API：  
...
2. 通过 +++https://admin.microsoft.com+++ 连接到 Microsoft 365 管理中心。
